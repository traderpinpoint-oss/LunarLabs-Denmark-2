      ...
      <div className="mx-auto max-w-6xl px-4 md:px-6">
        ...
        <div className="flex flex-col gap-6">
          {projects.map((p, i) => (
            ...
            >
              <div className={`relative min-h-72 md:min-h-96 ${i % 2 === 1 ? 'md:order-2' : ''}`}>
                <Image src={p.image} alt={p.alt} fill className="object-cover" sizes="(min-width: 768px) 50vw, 100vw" />
                <span className="absolute top-4 left-4 rounded-full bg-background/85 px-3 py-1 text-xs font-semibold tracking-widest uppercase backdrop-blur-sm">
                  Demo project
                </span>
                <div className="absolute bottom-4 left-4 rounded-2xl bg-background/85 px-4 py-3 backdrop-blur-sm">
              ...
              <div className="flex flex-col justify-between gap-10 p-7 md:p-10">
                <div>
                  <div className="mb-4 flex items-center gap-3 text-xs font-semibold tracking-widest text-muted-foreground uppercase">
                    <span>{p.year}</span>
                    <span className="size-1 rounded-full bg-accent" aria-hidden="true" />
                    <span>{p.kind}</span>
                  </div>
                  <h3 className="font-display mb-4 text-3xl font-bold tracking-tight md:text-4xl">
                    {p.title}
                  </h3>
                  <p className="leading-relaxed text-muted-foreground text-pretty">{p.body}</p>
                </div>
